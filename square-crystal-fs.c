...
void main(void)
{
    ...
    // approximation to |Du|_1
    vec4 rat1 = 1.0 / (epsilon * epsilon + D1 * D1);
    vec4 sqrtrat1 = sqrt(rat1);
    vec4 rat2 = 1.0 / (epsilon * epsilon + D2 * D2);
    vec4 sqrtrat2 = sqrt(rat2);
    vec4 op = (sqrtrat1 * (1.0 - D1 * D1 * rat1) + epsilon) * X11
            + (sqrtrat2 * (1.0 - D2 * D2 * rat2) + epsilon) * X22;
